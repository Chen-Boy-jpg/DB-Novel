...
import axios, {
  ...
  export const requestWithTimeout =
    ...
    async (endpoint: string, config: AxiosRequestConfig) => {
      ...
      const finalBaseURL = removeApiSuffix && api.defaults.baseURL?.endsWith('/api')
  ...
  export const put = <T>(endpoint: string, payload: T, removeApiSuffix: boolean = false) =>
    requestWithTimeout(50000, removeApiSuffix).bind(null, endpoint, {
      method: 'PUT',
      data: payload,
    });
  
  export const post = <T>(endpoint: string, payload: T, removeApiSuffix: boolean = false) =>
    requestWithTimeout(50000, removeApiSuffix).bind(null, endpoint, {
      method: 'POST',
      data: payload,
    });
  ...
  export const postWithResponse = async <T>(
    ...
    try {
      const response = await requestWithTimeout(50000, removeApiSuffix).bind(null, endpoint, {
        method: 'POST',
        data: payload,
      })();
      return response;
